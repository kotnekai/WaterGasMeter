 ...
 * @author admin
 */
public class PreStorageSave2Activity extends BaseActivity {
    ...
    public static Intent makeIntent(Context context, int pageType) {
        Intent intent = new Intent(context, PreStorageSave2Activity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = PreStorageSave2Activity.this;
        initView();
        initData();
    }
       ...
       String typeJsonStr =  PreferencesUtils.getString(CommonParams.METTER_TYPE_JSON);
        ...
        if (list.size()>0) {
            for (MeterTypeModel model : list) {
                ...
                mViewPagerFragments.add(ReadAndReChargeFragment.newInstance(model.getId(), model.getName_zh(), pageType));
            }
        }
        else
        {
